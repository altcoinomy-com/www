...
$(window).on("load", function () {
    ...
    function e() {
        var e = o.scrollTop(), t = D.$welcome, a = D.$whatWeDo, n = D.$partners, s = D.$projects,
            r = D.$team, l = D.$map, s = D.$nav;

        // h.removeClass("active"), e > t.offset().top && e < n.offset().top ? $("[data-to=what-we-do]").addClass("active") : e > n.offset().top && e < r.offset().top ? $("[data-to=projects]").addClass("active") : e > s.offset().top && e < l.offset().top ? $("[data-to=team]").addClass("active") : e > r.offset().top && $("[data-to=map]").addClass("active")
    }
    function t(e, t) {
    ...
    var a = [{
        ...
        Pascal: {
            name: "Pascal​ ​Minger",
            role: "Compliance​ ​Officer",
            text: "With a background in information technology Pascal started his career with Arthur Andersen as a developer and later graduated with an MBA from the University of Cumbria. As the former Managing Director and Chief Compliance Officer of the Swiss based FINMA regulated financial intermediary - Express Union International, he brings with him a wide array of experience in regulatory matters and business development. He is also an avid crypto enthusiast."
        },
        Jean: {
    ...
    function scrollToWithAnimation(element) {
        if(element =="#what")
        {
            $('html, body').animate({
                scrollTop: D.$welcome.height() - 200 - s.height()
            },  5500);

        }
        else if (element == "#projects"){
            $('html, body').animate({
                scrollTop: (D.$welcome.height() + D.$whatWeDo.height() + D.$partners.height() + 400 ) - s.height() 
            },  5500);
        }
        else if (element == "#team"){
            $('html, body').animate({
                scrollTop: (D.$welcome.height() + D.$whatWeDo.height() + D.$partners.height() + 400 + D.$projects.height() + 330) - s.height() 
            },  5500);
        }
        else if (element == "#map"){
            $('html, body').animate({
                scrollTop: (D.$welcome.height() + D.$whatWeDo.height() + D.$partners.height() + 400 + D.$projects.height() + 330 + D.$team.height() + 500 ) - s.height() 
            },  5500);
        }
    ...
    $(function () {
        $(".menu__item").click(function () {
            c.removeClass("blocked"), l.removeClass("shown");
            var scrollTo = $(this).attr('scroll-to');
            scrollToWithAnimation('#' + scrollTo);
            l.removeClass("shown");
            l.css("z-index", "-1"), s.css("z-index", "100");
        });
